...
def test_send_success(client: Any) -> None:
    ...
    with (
        ...
        response = client.post(
            "/send",
            json={
                "link": "https://audiobookbay.lu/book-page",
                "title": "Test Book",
            },
        )
        assert response.status_code == 200
...
def test_send_upstream_connection_failure(client: Any) -> None:
    """Test that explicit ConnectionError during magnet extraction returns 503."""
    with patch("audiobook_automated.routes.extract_magnet_link") as mock_extract:
        mock_extract.side_effect = ConnectionError("Upstream Down")
        response = client.post(
            "/send",
            json={
                "link": "https://audiobookbay.lu/book-page",
        ...
        assert response.json == {"message": "Upstream service unavailable. Please try again later."}
